...
def load_data(img_path, label_path, shuffle=True):
...
class dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return img, label
train_set, valid_set = load_data(img_path = "./train/train",label_path = "train/train/label.csv")
...
for epoch in range(epochs):
    ...
    train_tl.append(float(np.mean(train_loss)))
    train_ta.append(float(np.mean(train_acc)))
    model.eval()
    with torch.no_grad():
        valid_loss = []
        valid_acc = []
        ...
        valid_tl.append(float(np.mean(valid_loss)))
        valid_ta.append(float(np.mean(valid_acc)))
statistic = [train_tl,train_ta,valid_tl,valid_ta]
df = pd.DataFrame(statistic)
df.to_csv("statitic.csv",header=None)
torch.save(model.state_dict(), 'CNN_pytorch.pkl')